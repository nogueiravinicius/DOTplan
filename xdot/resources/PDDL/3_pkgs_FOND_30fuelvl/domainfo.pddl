...
(:action land
 ...
 :effect (and (not(free))(grounded) (not(onair)) (quadfuel ?fuel1)(not(quadfuel ?fuel2))(oneof (operating)(and(recovering)(not(operating))))))

(:action take-off
 :parameters (?fuel1 ?fuel2 - fuellvl)
 :precondition (and (grounded)(quadfuel ?fuel2)(fuelmapping ?fuel1 ?fuel2)(free)(operating))
 :effect (and (not(free))(onair) (not(grounded)) (quadfuel ?fuel1)(not(quadfuel ?fuel2))(oneof (operating)(and(recovering)(not(operating))))))
...
(:action move-with-w0-to
 ...
 :effect (and (free)(not (quadat ?wp1)) (quadat ?wp2)(not(quadfuel ?fuel3))
		(oneof (quadfuel ?fuel2)
		       (quadfuel ?fuel1)
                       (and(quadfuel ?fuel1)(recovering)(not(operating)))
			       )))
