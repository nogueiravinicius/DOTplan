...
(:action refuel
 :parameters (?wp1 - waypoint ?fuel1 ?fuel2 - fuellvl ?refuel1 ?refuel2 - refuellvl)
 :precondition (and(quadat ?wp1)(stationat ?wp1)(quadfuel ?fuel1)(maximumfuel ?fuel2)(grounded)(refuelstat ?refuel2)(refuelmapping ?refuel1 ?refuel2))
 :effect (and (free)(quadfuel ?fuel2)(not(quadfuel ?fuel1))(refuelstat ?refuel1)(not(refuelstat ?refuel2))))

(:action deliver-package
 :parameters (?pkg - package ?wp1 ?wp2 - waypoint)
 :precondition (and (pkgat ?pkg ?wp1) (quadat ?wp2) (deliverat ?pkg ?wp2) (inquad ?pkg)(grounded))
 :effect (and (free)(not (pkgat ?pkg ?wp1)) (pkgat ?pkg ?wp2)(not(inquad ?pkg)) (delivered ?pkg)))
)
