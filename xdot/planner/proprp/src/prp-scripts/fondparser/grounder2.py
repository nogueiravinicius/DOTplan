...
class GroundProblem(Problem):
    ...
    def _action_to_operator(self, action, assignment, fluent_dict):
        ...
        #if "refuel" is action.name:
        #    print action.effect
        return Operator(op_name, op_params, op_precond, op_observe, op_effect)

    def _create_operators(self, fluent_dict, action_list):
...
#        print action_list
        for act in action_list:
            try:
                ...
                continue
#           print "UUUU",params
            act_name=params.pop(0)
            for a in self.actions:
                if act_name == a.name :
                 # print "ACHOU ", act_name, " = ", a.name
#                 var_names=self.get_var_names(a.parameters, a)
#                 print a.parameters[0]
                 assignment = {var_name: val for var_name, val in zip([item[0] for item in a.parameters], params)}
#                 print "--------", assignment 
                 oppp=(self._action_to_operator(a, assignment, fluent_dict))
                 self.operators.add(oppp)
#                 print oppp
...
#        print self.operators.pop()

    def get_var_names(self, params, action=None):
        d = self._create_param_dict(params)
    ...
    def create_operators(self,action_list):
        ...
        self._create_operators(fluent_dict,action_list)
