...
class Problem(object):
    ...
    def _add_objects(self, object_list):
        ...
        for obj, t in object_list:
            ...
            self.obj_to_type[obj].add(t)
            k = t
           
            while k in self.parent_types:
                if self.parent_types[k] not in self.type_to_obj:
                    self.type_to_obj[self.parent_types[k]] = set([])
                self.type_to_obj[self.parent_types[k]].add(obj)
                self.obj_to_type[obj].add(self.parent_types[k])
